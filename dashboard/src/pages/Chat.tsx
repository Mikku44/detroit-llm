...
export const Model: FC = () => {
  ...
  useEffect(() => {
    ...
    if (!token) {
      alert("ไม่พบ Session Token กรุณาเข้าสู่ระบบใหม่อีกครั้ง");
    }
    // เช็กเสร็จแล้วปิด Loading
    setIsLoading(false);
  }, []);
...
// ✅ แก้ไข useAuiState ให้ใช้ Selector Function แบบถูกวิธี
const useAttachmentSrc = () => {
  ...
  const blobSrc = useFileSrc(file);
  return blobSrc ?? rawSrc;
};

const ModelAttachmentUI: FC = () => {
  const aui = useAui();
  ...
  return (
    <AttachmentPrimitive.Root className="group/attachment relative">
      ...
      {isComposer && (
        <AttachmentPrimitive.Remove className="absolute -right-1.5 -top-1.5 flex size-7 items-center justify-center rounded-full border border-[#e5e5e5] bg-white text-[#6b6b6b] transition-all hover:bg-[#f5f5f5] hover:text-[#0d0d0d] dark:border-[#3a3a3a] dark:bg-[#1a1a1a] dark:text-[#9a9a9a] dark:hover:bg-[#252525] dark:hover:text-white">
